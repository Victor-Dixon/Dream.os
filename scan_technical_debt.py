...
class TechnicalDebtScanner:
    ...
    def generate_report(self, detailed: bool = True) -> str:
        ...
        report.append(f"**Total Markers Found:** {sum(len(items) for items in self.results.values())}")
        report.append("")
        ...
        for marker_type in sorted(self.results.keys(), key=lambda x: len(self.results[x]), reverse=True):
        ...
        # Detailed breakdown
        if detailed:
            for marker_type, items in sorted(self.results.items(), key=lambda x: len(x[1]), reverse=True):
                if not items:
                    continue
...
def main():
    ...
    parser.add_argument("--type", choices=list(DEBT_MARKERS.keys()), help="Specific marker type to scan for")
