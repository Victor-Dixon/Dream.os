        ...
        try:
            ...
            try:
                result = subprocess.run([
                    ...
                    '--agent', self.captain_agent_id,
                    '--message', message,
                    '--sender', 'PR Monitoring System',
            ...
            if success:
                logger.info(f"PR notification sent to Captain Agent-4 for PR #{pr.number}")
                return True
            else:
            ...
            for pr in open_prs:
                ...
                if pr_id not in self.known_prs:
                    ...
                    if pr.ready_for_merge:
                        ...
                        logger.info(f"New PR #{pr.number} detected but not ready for merge: {pr.title}")
