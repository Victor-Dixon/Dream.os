            ...
            if summary.get('status') == 'no_data':
                ...
                for name, data in summary.items():
                    if isinstance(data, dict) and 'avg' in data:
                        print(f"{name}: avg={data['avg']:.2f}, min={data['min']:.2f}, max={data['max']:.2f}")
...
def handle_optimize_command(args):
    ...
    if args.opt_action == 'start':
        ...
        if args.format == 'json':
            ...
            print(f"Rules: {summary.get('enabled_rules', 0)}/{summary.get('rules_count', 0)} enabled")
        ...
        if args.format == 'json':
            ...
            if not history:
                ...
                for opt in history[-10:]:  # Show last 10
                    ...
                    print(f"{status} {opt['rule_name']}: {opt['improvement_percent']:.2f}% improvement")
    ...
    if args.dashboard_action == 'summary':
        ...
        if args.format == 'json':
            ...
            if trends.get('status') == 'no_data':
                ...
                print(f"CPU: {cpu_trend.get('current', 0):.1f}% (trend: {cpu_trend.get('trend', 'unknown')})")
                print(f"Memory: {mem_trend.get('current', 0):.1f}% (trend: {mem_trend.get('trend', 'unknown')})")
