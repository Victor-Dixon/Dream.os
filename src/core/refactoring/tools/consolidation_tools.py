...
class ConsolidationTools:
    ...
    def consolidate_directory(
        ...
        """
        Consolidate multiple files in a directory into a single file.

        Args:
            source_dir: Directory containing files to consolidate
            target_file: Path for consolidated output file
        ...
        """
        try:
            source_path = self.unified_imports.Path(source_dir)
            ...
            if backup:
                ...
                if backup_path.exists():
                    self.unified_imports.shutil.rmtree(backup_path)
                self.unified_imports.shutil.copytree(source_path, backup_path)
            ...
            consolidated = self._generate_consolidated_content(py_files, source_dir)
    ...
    def _generate_consolidated_content(
        self,
        py_files: list,
        base_dir: str
    ) -> str:
        """Generate consolidated file content from multiple Python files."""
        header = f'''"""
Consolidated Module - Auto-generated
====================================

This module consolidates functionality from: {base_dir}
Generated by ConsolidationTools

V2 Compliance: Consolidated for maintainability
"""
...
'''
        # Extract unique imports and functions
        all_imports = set()
        all_functions = []
        all_classes = []
        ...
        for py_file in py_files:
            ...
            try:
                ...
                # Basic parsing - extract imports, functions, classes
                for line in content.split('\n'):
                    stripped = line.strip()
                    if stripped.startswith(('import ', 'from ')):
                        all_imports.add(stripped)
                    elif stripped.startswith('def ') and not stripped.startswith('def _'):
                        all_functions.append(f"# From {py_file.name}\n{line}")
                    elif stripped.startswith('class '):
                        all_classes.append(f"# From {py_file.name}\n{line}")
            except Exception:
                continue
...
def initialize_consolidated_module() -> None:
    """Initialize consolidated module."""
    logger.info("Consolidated module initialized")

initialize_consolidated_module()
'''

        return header + imports_str + classes_str + '\n\n' + functions_str + footer