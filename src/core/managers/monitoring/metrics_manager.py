...
from .base_monitoring_manager import BaseMonitoringManager, MetricType
...
class MetricsManager(BaseMonitoringManager):
    ...
    def _get_metric_aggregation(self, context: ManagerContext, payload: Dict[str, Any]) -> ManagerResult:
        ...
        try:
            ...
            values = [entry["value"] for entry in filtered_history if isinstance(entry["value"], (int, float))]
    ...
    def _get_metric_trends(self, context: ManagerContext, payload: Dict[str, Any]) -> ManagerResult:
        ...
        try:
            ...
            values = [entry["value"] for entry in filtered_history if isinstance(entry["value"], (int, float))]
            timestamps = [entry["timestamp"] for entry in filtered_history if isinstance(entry["value"], (int, float))]
    ...
    def _export_metrics(self, context: ManagerContext, payload: Dict[str, Any]) -> ManagerResult:
        ...
        try:
            ...
            # Filter metrics
            if metric_names:
                export_data = {name: self.metrics.get(name) for name in metric_names if name in self.metrics}
            else:
            ...
            if export_format == "json":
                ...
                for name, metric in export_data.items():
                    if isinstance(metric.get("value"), (int, float)):
                        csv_lines.append(f"{name},{metric['timestamp']},{metric['value']},{metric.get('type', 'unknown')}")
                export_string = "\n".join(csv_lines)
            else:
                return ManagerResult(
                    success=False,
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        base_status.update({
            ...
            "avg_history_per_metric": sum(len(history) for history in self.metric_history.values()) / max(len(self.metric_history), 1),
        })
        return base_status
