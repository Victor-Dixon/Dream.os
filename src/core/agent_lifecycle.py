...
class AgentLifecycle:
    ...
    def start_mission(self, mission_name: str, priority: str = "MEDIUM") -> None:
        ...
        print(
            f"✅ {self.agent_id}: Mission started - {mission_name} (Priority: {priority})")
    ...
    def complete_task(self, task_name: str, points: int = 0) -> None:
        ...
        # Add points
        self.status['points_earned'] = self.status.get(
        ...
        print(
            f"✅ {self.agent_id}: Task complete - {task_name} (+{points} pts, total: {total_points})")
    ...
    def complete_mission(self) -> None:
        ...
        print(
            f"🎯 {self.agent_id}: Mission COMPLETE - {mission} ({points} total points)")
    ...
    def end_cycle(self, commit: bool = False) -> None:
        ...
        if commit:
            if self._commit_to_git():
                print(
                    f"✅ {self.agent_id}: Cycle {cycle_num} ended (committed to git)")
            else:
                print(
                    f"⚠️ {self.agent_id}: Cycle {cycle_num} ended (git commit failed)")
        else:
