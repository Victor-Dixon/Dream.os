...
class AutoGasPipelineSystem:
        ...
        while self.monitoring_active:
            ...
            for agent_id, agent in self.agents.items():
                try:
                    ...
                    handle_pipeline_error(agent_id, e, {"cycle": cycle, "progress": progress if 'progress' in locals() else None})
    ...
    def _send_auto_gas(self, agent_id: str, reason: str, progress: float):
        ...
        if success:
            ...
            print(f"⛽ AUTO-GAS SENT: {agent_id} ({progress:.1f}%) → {next_agent_id} [{reason}]")
        else:
    ...
    def force_gas_send(self, agent_id: str, reason: str = "MANUAL"):
        ...
        progress = calculate_progress(agent_id, self.agents[agent_id], self.workspace_path)
        self._send_auto_gas(agent_id, reason, progress)
