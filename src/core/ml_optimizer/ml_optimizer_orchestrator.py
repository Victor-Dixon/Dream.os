...
class VectorDatabaseMLOptimizer:
    ...
    def get_models_by_strategy(self, strategy: MLStrategy) -> List[MLModel]:
        ...
        return [model for model in self.models.values() if model.strategy == strategy]
...
# Factory functions
def create_vector_database_ml_optimizer(
    config: Optional[MLConfiguration] = None,
) -> VectorDatabaseMLOptimizer:
    """Create a vector database ML optimizer."""
    return VectorDatabaseMLOptimizer(config)


def get_vector_database_ml_optimizer(
    config: Optional[MLConfiguration] = None,
) -> VectorDatabaseMLOptimizer:
