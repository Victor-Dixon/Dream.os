...
class InboxRotationManager:
    ...
    def __init__(self, max_size_kb: int = MAX_INBOX_SIZE_KB, 
    ...
    def should_rotate(self, inbox_path: Path) -> bool:
        ...
        if size_kb > self.max_size_kb:
            self.logger.info(f"Inbox {inbox_path.name} exceeds size limit ({size_kb:.1f}KB > {self.max_size_kb}KB)")
            return True
        ...
        try:
            with open(inbox_path, 'r', encoding='utf-8') as f:
                ...
                if message_count > self.max_messages:
                    self.logger.info(f"Inbox {inbox_path.name} exceeds message limit ({message_count} > {self.max_messages})")
                    return True
    ...
    def cleanup_old_archives(self, max_age_days: int = 30, base_dir: str = "agent_workspaces") -> int:
