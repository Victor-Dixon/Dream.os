...
LEGACY_MAP = {
    ...
    "EnhancedIntegrationOrchestrator": ["integration.init", "integration.coordinate", "integration.optimize"],
    "SwarmCoordinationEnhancer": ["swarm.init", "swarm.coordinate", "swarm.monitor"],
    "EmergencyInterventionOrchestrator": ["emergency.init", "emergency.assess", "emergency.intervene"],
    "MessagingOptimizationOrchestrator": ["messaging.init", "messaging.optimize", "messaging.deliver"],
}

class LegacyOrchestratorAdapter(Orchestrator):
    """Wrap a legacy Orchestrator interface to the new CoreOrchestrator without breaking callers."""
