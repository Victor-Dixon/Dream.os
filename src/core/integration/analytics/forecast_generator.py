...
from ..vector_integration_models import (
    PerformanceForecast, PerformanceMetrics, create_performance_forecast
)
...
class ForecastGenerator:
    ...
    def generate_forecast(self, metrics_data: List[PerformanceMetrics],
        ...
        try:
            ...
            # Generate forecast
            forecast_values, confidence_interval, model_accuracy = self._generate_simple_forecast(metric_values)
            ...
            self.logger.debug(f"Forecast generated for {metric_name} with accuracy {model_accuracy:.2f}")
            return forecast
    ...
    def _generate_simple_forecast(self, values: List[float]) -> Tuple[List[float], Tuple[float, float], float]:
        ...
        residuals = [y - (slope * x + intercept) for x, y in zip(x_values, recent_values)]
    ...
    def generate_multi_metric_forecast(self, metrics_data: List[PerformanceMetrics]) -> Dict[str, PerformanceForecast]:
