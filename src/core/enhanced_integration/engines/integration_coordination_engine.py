...
class IntegrationCoordinationEngine:
    ...
    async def coordinate_integrations(
        ...
        try:
            self.logger.info(
                f"Coordinating {len(tasks)} integration tasks with strategy: {strategy}"
            )
            ...
            self.logger.info(
                f"Coordination completed: {results['completed']}/{results['total']} tasks successful"
            )
            return results
    ...
    async def _execute_parallel(self, tasks: list[IntegrationTask]) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "total": len(tasks),
                "execution_time": (
                    max(task.execution_time for task in tasks if task.execution_time)
                    if tasks
                    else 0
                ),
            }
