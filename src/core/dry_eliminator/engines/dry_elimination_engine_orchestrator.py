...
from ..dry_eliminator_models import (
    DRYEliminatorConfig, DRYViolation, EliminationResult, EliminationStrategy,
...
from .violation_detection_engine import ViolationDetectionEngine, create_violation_detection_engine
from .elimination_strategy_engine import EliminationStrategyEngine, create_elimination_strategy_engine
from .metrics_reporting_engine import MetricsReportingEngine, create_metrics_reporting_engine
...
class DRYEliminationEngineOrchestrator:
    ...
    def run_full_analysis(self, project_root: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            summary['analysis_summary'] = self.analysis_engine.get_analysis_summary(self.python_files)
    ...
    def _determine_elimination_strategy(self, violation: DRYViolation) -> EliminationStrategy:
        ...
        elif violation.violation_type.value in ['duplicate_imports', 'duplicate_constants']:
            return EliminationStrategy.CONSOLIDATE
        elif violation.violation_type.value in ['duplicate_methods', 'duplicate_code_blocks']:
    ...
    def get_analysis_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "analysis_in_progress": self.metrics_engine.start_time is not None and self.metrics_engine.end_time is None
        }


# Factory function for dependency injection
def create_dry_elimination_engine_orchestrator(config: Optional[DRYEliminatorConfig] = None) -> DRYEliminationEngineOrchestrator:
    """Factory function to create DRY elimination engine orchestrator with optional configuration"""
    return DRYEliminationEngineOrchestrator(config)


# Export for DI
__all__ = ['DRYEliminationEngineOrchestrator', 'create_dry_elimination_engine_orchestrator']
