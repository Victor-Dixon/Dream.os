...
class MetricsReportingEngine:
    ...
    def generate_summary_report(
        ...
        return {
            ...
            "performance_metrics": {
                "violations_per_file": (
                    self.metrics.violations_detected / self.metrics.total_files_analyzed
                    if self.metrics.total_files_analyzed > 0
                    else 0
                ),
                "lines_removed_per_violation": (
                    self.metrics.total_lines_removed / self.metrics.violations_detected
    ...
    def generate_detailed_report(
        ...
        report.append(
            f"  Total Potential Savings: {sum(v.potential_savings for v in violations)} lines"
        )
        report.append("")
