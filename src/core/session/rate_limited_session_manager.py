...
# SSOT: docs/recovery/recovery_registry.yaml#src-core-session-rate-limited-session-manager
# @registry docs/recovery/recovery_registry.yaml#src-core-session-rate-limited-session-manager
...
class RateLimitedSessionInfo(BaseSessionInfo):
    ...
    rate_limit_status: RateLimitStatus | None = field(
        default_factory=lambda: RateLimitStatus(requests_remaining=0)
    )
...
class RateLimitedSessionManager(BaseSessionManager):
    ...
    def __init__(self, config: dict[str, Any] | None = None, logger_name: str | None = None):
        """
        ...
        self.logger.info(
            f"Rate limiting: {self.requests_per_minute} req/min, burst: {self.burst_limit}"
        )

    def create_session(self, service_name: str, **kwargs) -> str | None:
        """
        ...
        try:
            ...
            if len(self.sessions) >= self.max_sessions:
                self.logger.warning(
                    f"Max sessions ({self.max_sessions}) reached, cleaning up expired"
                )
    ...
    def validate_session(self, session_id: str) -> bool:
        ...
        if session.status != "active":
            self.logger.warning(
                f"Session validation failed - {session_id} not active (status: {session.status})"
            )
            return False
    ...
    def wait_for_rate_limit_reset(self, service_name: str, session_id: str) -> None:
        ...
        if rate_limit.reset_time:
            ...
            if wait_time > 0:
                self.logger.info(
                    f"⏳ Waiting {wait_time:.1f}s for rate limit reset ({service_name})"
                )
    ...
    def handle_rate_limit_error(self, service_name: str, session_id: str) -> None:
        ...
        if service_name in self.rate_limits:
            ...
            self.logger.warning(
                f"🚨 Rate limit error detected for {service_name} - throttling enabled"
            )
    ...
    def get_rate_limit_status(self, service_name: str) -> dict[str, Any]:
        ...
        """
        if service_name not in self.rate_limits:
            return {"error": "Service not configured", "service_name": service_name}
