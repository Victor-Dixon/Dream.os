...
class EmergencyProtocols:
    ...
    Uses modular handlers to provide protocol capabilities
    ...
    def get_protocol(self, emergency_type: EmergencyType, 
                    severity: EmergencySeverity) -> Optional[InterventionProtocol]:
        """Get protocol for emergency type and severity."""
        return self.registry.get_protocol(emergency_type, severity)
    
    def execute_protocol(self, emergency_type: EmergencyType, 
        ...
        try:
            ...
            if not protocol:
                return {
                    'status': 'no_protocol',
                    'message': f'No protocol found for {emergency_type.value} with {severity.value} severity'
                }
    ...
    def list_protocols(self, emergency_type: Optional[EmergencyType] = None) -> List[InterventionProtocol]:
    ...
    def get_protocols_status(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                'protocol_types': {
                    emergency_type.value: len(self.registry.list_protocols(emergency_type))
