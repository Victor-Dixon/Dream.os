...
class EmergencyAnalyzer:
    ...
    def get_analysis_metrics(self) -> Dict[str, Any]:
        ...
        critical_count = sum(1 for a in self.analysis_history if a.get('risk_assessment', {}).get('risk_level') == 'critical')
        high_count = sum(1 for a in self.analysis_history if a.get('risk_assessment', {}).get('risk_level') == 'high')
        
        return {
            'total_analyses': total_analyses,
            'critical_emergencies': critical_count,
            'high_emergencies': high_count,
            'average_priority_score': sum(a.get('priority_score', 0) for a in self.analysis_history) / total_analyses
        }
