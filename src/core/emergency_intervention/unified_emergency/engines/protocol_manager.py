...
class ProtocolManager:
    ...
    def find_matching_protocols(
        ...
        for protocol in self.protocols.values():
            if (
                protocol.emergency_type == emergency.emergency_type
                and self._severity_matches(
                    emergency.severity, protocol.severity_threshold
                )
            ):
    ...
    def _severity_matches(
        self, emergency_severity: EmergencySeverity, threshold: EmergencySeverity
    ) -> bool:
    ...
    def get_protocol_summary(self) -> Dict[str, Any]:
        ...
        for protocol in self.protocols.values():
            ...
            summary["protocols_by_type"][type_name] = (
                summary["protocols_by_type"].get(type_name, 0) + 1
            )

            # Count by severity
            severity_name = protocol.severity_threshold.value
            summary["protocols_by_severity"][severity_name] = (
                summary["protocols_by_severity"].get(severity_name, 0) + 1
            )

            # Count by priority
            priority = protocol.priority
            summary["protocols_by_priority"][priority] = (
                summary["protocols_by_priority"].get(priority, 0) + 1
            )
