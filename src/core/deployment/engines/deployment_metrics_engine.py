...
class DeploymentMetricsEngine:
    ...
    def start_deployment_tracking(self, total_targets: int) -> DeploymentMetrics:
        ...
        self.current_metrics = create_deployment_metrics(
            total_targets=total_targets, start_time=datetime.now()
        )
    ...
    def update_target_completion(self, target: MassDeploymentTarget) -> None:
        ...
        try:
            ...
            self.logger.debug(
                f"Updated metrics: {self.current_metrics.completed_targets}/{self.current_metrics.total_targets} completed"
            )
    ...
    def finish_deployment_tracking(self) -> DeploymentMetrics | None:
        ...
        try:
            ...
            self.logger.info(
                f"Deployment tracking completed: {final_metrics.success_rate:.2%} success rate"
            )
        ...
        try:
            ...
            if self.current_metrics.total_targets > 0:
                progress_percentage = (
                    ...
                    / self.current_metrics.total_targets
                    * 100
                )
            ...
            return {
                ...
                "success_rate": (
                    round(self.current_metrics.success_rate * 100, 2)
                    if self.current_metrics.success_rate
                    else 0
                ),
                "average_execution_time": (
                    round(self.current_metrics.average_execution_time, 3)
                    if self.current_metrics.average_execution_time
                    else 0
                ),
                "throughput": (
                    round(self.current_metrics.throughput, 3)
                    if self.current_metrics.throughput
                    else 0
                ),
                "elapsed_time": (
                    (datetime.now() - self.current_metrics.start_time).total_seconds()
                ),
            }
