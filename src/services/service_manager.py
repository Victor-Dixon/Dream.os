...
class ServiceManager:
    ...
    def _run_service_foreground(self, service_name: str):
        ...
        try:
            ...
            logger.info(f"Running {service_name} script: {script_path}")

            # Import and run the service script
            ...
            if result.returncode == 0:
                ...
                logger.error(f"Service {service_name} failed: {result.stderr}")
                return False

        except Exception as e:
            logger.error(f"Failed to run {service_name} in foreground: {e}")
            return False
