...
from .models import StatusEntry, StatusType, StatusSummary, AgentStatus, PerformanceMetrics, StatusTrackerModels
...
class MessagingStatusTracker:
    ...
    def track_message_sent(self, message_id: str, agent_id: str, details: Dict[str, Any] = None) -> None:
        """Track a message sent event."""
        entry = StatusTrackerModels.create_status_entry(
            StatusType.MESSAGE_SENT,
            message_id,
            agent_id,
            details or {},
            True
    ...
    def track_message_delivered(self, message_id: str, agent_id: str, details: Dict[str, Any] = None) -> None:
        """Track a message delivered event."""
        entry = StatusTrackerModels.create_status_entry(
            StatusType.MESSAGE_DELIVERED,
            message_id,
            agent_id,
            details or {},
            True
    ...
    def track_message_failed(self, message_id: str, agent_id: str, error: str, details: Dict[str, Any] = None) -> None:
    ...
    def track_performance_metric(self, metric_name: str, value: float, details: Dict[str, Any] = None) -> None:
    ...
    def get_status_summary(self, time_window: Optional[timedelta] = None) -> StatusSummary:
        ...
        return StatusTrackerModels.create_status_summary(
            ...
            successful_messages=performance_metrics.total_requests - performance_metrics.error_count,
            failed_messages=performance_metrics.error_count,
            average_response_time=performance_metrics.average_response_time
        )
    
    def get_agent_status(self, agent_id: str, time_window: Optional[timedelta] = None) -> Optional[AgentStatus]:
    ...
    def generate_report(self, report_type: str = 'status', time_window: Optional[timedelta] = None) -> Dict[str, Any]:
    ...
    def get_agent_report(self, agent_id: str, time_window: Optional[timedelta] = None) -> Dict[str, Any]:
    ...
    def clear_old_data(self, days_to_keep: int = 30) -> int:
        ...
        old_entries = [entry for entry in self.analytics.status_entries if entry.timestamp < cutoff_time]
        count = len(old_entries)
        
        # Remove old entries
        self.analytics.status_entries = [
            entry for entry in self.analytics.status_entries 
