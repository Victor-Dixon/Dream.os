...
class StatusAnalytics:
    ...
    def get_agent_success_rate(self, agent_id: str, time_window: Optional[timedelta] = None) -> float:
    ...
    def get_agent_statistics(self, time_window: Optional[timedelta] = None) -> Dict[str, Dict[str, Any]]:
        ...
        for entry in entries:
            ...
            if (agent_stats[agent_id]['last_activity'] is None or 
                entry.timestamp > agent_stats[agent_id]['last_activity']):
    ...
    def get_error_analysis(self, time_window: Optional[timedelta] = None) -> Dict[str, Any]:
    ...
    def _filter_by_time_window(self, time_window: Optional[timedelta]) -> List[StatusEntry]:
        """Filter entries by time window."""
        if time_window is None:
            return self.status_entries
        
        cutoff_time = datetime.now() - time_window
        return [entry for entry in self.status_entries if entry.timestamp >= cutoff_time]
    
    def _filter_by_agent_and_time(self, agent_id: str, time_window: Optional[timedelta]) -> List[StatusEntry]:
        """Filter entries by agent and time window."""
        entries = self._filter_by_time_window(time_window)
        return [entry for entry in entries if entry.agent_id == agent_id]
    
    def _calculate_error_trends(self, error_entries: List[StatusEntry]) -> List[Dict[str, Any]]:
