...
from .models import StatusEntry, StatusType, StatusSummary, AgentStatus, PerformanceMetrics
...
class StatusReporter:
    ...
    def generate_status_report(self, time_window: Optional[timedelta] = None) -> Dict[str, Any]:
    ...
    def generate_agent_report(self, agent_id: str, time_window: Optional[timedelta] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            'success_rate': self.analytics.get_agent_success_rate(agent_id, time_window),
            'is_online': agent_data.get('last_activity', datetime.min) > datetime.now() - timedelta(hours=1)
        }
    ...
    def generate_error_report(self, time_window: Optional[timedelta] = None) -> Dict[str, Any]:
    ...
    def _generate_error_recommendations(self, error_analysis: Dict[str, Any]) -> List[str]:
        ...
        if error_analysis['common_errors']:
            ...
            recommendations.append(f"Most common error: {top_error[0]} ({top_error[1]} occurrences). Focus on fixing this issue.")
        ...
        if error_analysis['error_trends']:
            ...
            if len(recent_errors) >= 2:
                if recent_errors[-1]['error_count'] > recent_errors[-2]['error_count']:
                    recommendations.append("Error count is increasing. Investigate recent changes.")
