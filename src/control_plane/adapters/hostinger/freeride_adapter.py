...
Hostinger Site Adapter - FreeRideInvestor
=========================================

Provides a narrow, allowlisted interface to interact with the FreeRideInvestor
WordPress instance hosted on Hostinger. Operations are intentionally scoped to
low-risk actions and must be invoked via the control plane with policy checks.
"""
...
class FreeRideHostingerAdapter:
    """
    Hostinger adapter for FreeRideInvestor.

    Allowlisted ops (suggested):
    - cache_flush
    - permalinks_refresh (requires wp-cli endpoint or worker)
    - deploy_preset (only predefined path/port/user)
    ...
    def __init__(
        ...
        self.allowed_ops = {
            ...
            # "deploy_preset": self._op_deploy_preset,  # wire when ready
        }
    ...
    def last_deploy(self) -> Dict[str, Any]:
        """
        Reads last deploy metadata from a local artifact if available.
        This avoids storing secrets; it only reports known deploy info.
        """
        meta_path = Path("runtime/deploy_logs/freerideinvestor_last.json")
    ...
    def run_allowed(self, op: str, payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        Dispatch an allowlisted operation.
        """
    ...
    def _op_permalinks_refresh(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """
        Placeholder for permalinks refresh via a secured endpoint or worker.
        """
    ...
    def _op_post_daily_plan(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Requires: wp_user/wp_app_password set on adapter init.
        Payload example: {"title": "...", "content": "...", "categories": [id]}
        """
    ...
    # def _op_deploy_preset(self, payload: Dict[str, Any]) -> Dict[str, Any]:
    #     """
    #     If/when wired: call a safe deploy worker with fixed params (no arbitrary paths).
    #     """
    #     return {"ok": False, "error": "deploy_preset not wired yet"}


# Factory helper (for DI later)
def get_freeride_adapter() -> SiteAdapter:
    # These should be sourced from env/config, not hardcoded.
    base_url = "https://freerideinvestor.com"
    wp_user = None
    wp_app_password = None
    return FreeRideHostingerAdapter(base_url=base_url, wp_user=wp_user, wp_app_password=wp_app_password)
