...
Hostinger Site Adapter - Prismblossom
=====================================

Provides a narrow, allowlisted interface to interact with the Prismblossom
WordPress instance hosted on Hostinger. Operations are scoped to low-risk actions
and must be invoked via the control plane with policy checks.
"""
...
class PrismblossomHostingerAdapter:
    """
    Hostinger adapter for Prismblossom.
    ...
    def last_deploy(self) -> Dict[str, Any]:
        meta_path = Path("runtime/deploy_logs/prismblossom_last.json")
...
def get_prismblossom_adapter() -> SiteAdapter:
    base_url = "https://prismblossom.online"
    wp_user = None
    wp_app_password = None
    return PrismblossomHostingerAdapter(base_url=base_url, wp_user=wp_user, wp_app_password=wp_app_password)
