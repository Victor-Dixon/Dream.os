...
class ModerationModules:
    ...
    def _setup_auto_moderation(self):
        ...
        self.auto_moderation_rules = {
            "spam_detection": {
                "enabled": (
                    self.config.get("moderation", {}).get("auto_moderation", True)
                ),
            ...
            "profanity_filter": {
                "enabled": (
                    self.config.get("moderation", {}).get("profanity_filter", True)
                ),
                "action": "delete_message",
            },
            "raid_protection": {
                "enabled": (
                    self.config.get("moderation", {}).get("raid_protection", True)
                ),
    ...
    async def check_spam(self, message: discord.Message) -> bool:
        ...
        async for msg in channel.history(limit=10):
            if (
                ...
                and msg.created_at > datetime.utcnow() - timedelta(seconds=60)
            ):
        ...
        if (
            len(recent_messages)
            >= self.auto_moderation_rules["spam_detection"]["threshold"]
        ):
            return True
    ...
    async def check_raid_protection(self, guild: discord.Guild) -> bool:
        ...
        for member in guild.members:
            if member.joined_at and member.joined_at > datetime.utcnow() - timedelta(
                seconds=300
            ):
        ...
        if (
            len(recent_members)
            >= self.auto_moderation_rules["raid_protection"]["new_member_threshold"]
        ):
            return True
    ...
    async def handle_profanity(self, message: discord.Message) -> bool:
        ...
        try:
            ...
            if action == "delete_message":
                ...
                try:
                    await message.author.send(
                        "Your message was deleted due to inappropriate content."
                    )
                except:
                    pass
    ...
    def _log_moderation_action(
        ...
        get_logger(__name__).info(
            f"Moderation action logged: {action} on {target} by {moderator}"
        )
    ...
    async def warn_member(
        ...
        try:
            # Log the warning
            self._log_moderation_action(
                action="warn", target=member, moderator=moderator, reason=reason
            )
            ...
            get_logger(__name__).info(
                f"Warned member: {member.display_name} for: {reason}"
            )
            return True

        except Exception as e:
            get_logger(__name__).error(
                f"Failed to warn member {member.display_name}: {e}"
            )
            return False
    ...
    async def mute_member(
        ...
        try:
            ...
            get_logger(__name__).info(
                f"Muted member: {member.display_name} for {duration}"
            )
            return True

        except Exception as e:
            get_logger(__name__).error(
                f"Failed to mute member {member.display_name}: {e}"
            )
            return False
    ...
    async def unmute_member(
        ...
        try:
            ...
            # Log the action
            self._log_moderation_action(
                action="unmute", target=member, moderator=moderator, reason=reason
            )
            ...
            get_logger(__name__).error(
                f"Failed to unmute member {member.display_name}: {e}"
            )
            return False
