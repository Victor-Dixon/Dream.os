...
class MessageRepository:
    ...
    def get_message_history(
        ...
        if agent_id:
            messages = [
                m for m in messages 
                if m.get('to') == agent_id or m.get('from') == agent_id
            ]
    ...
    def get_messages_by_sender(self, sender_id: str) -> List[Dict[str, Any]]:
        ...
        sender_messages = [
            m for m in messages 
            if m.get('from') == sender_id
        ]
        
        sender_messages.sort(
            key=lambda x: x.get('timestamp', ''), 
            reverse=True
        )
    ...
    def get_message_count(self, agent_id: Optional[str] = None) -> int:
        """
