...
class SignalDetectionEngine:
    ...
    def check_entry_signals(self, row: pd.Series) -> Tuple[bool, bool]:
        ...
        cooldown_ok = (self.last_exit_bar == -999) or (self.current_bar - self.last_exit_bar >= self.config.cooldown_bars)
    ...
    def _check_long_signal(self, row: pd.Series) -> bool:
        ...
        try:
            ...
            long_sig = (
                ...
                (row['rsi'] > row['rsi']) and  # RSI crossover up (simplified)
                row['in_session'] and
                row['vol_ok']
            )
    ...
    def _check_short_signal(self, row: pd.Series) -> bool:
        ...
        try:
            ...
            short_sig = (
                ...
                (row['rsi'] < row['rsi']) and  # RSI crossover down (simplified)
                row['in_session'] and
                row['vol_ok']
            )
    ...
    def check_exit_signals(self, row: pd.Series, position_side: TradeSide, 
                          entry_price: float, stop_price: float, target_price: float) -> Optional[str]:
    ...
    def calculate_target_price(self, entry_price: float, stop_distance: float, side: TradeSide) -> float:
    ...
    def calculate_stop_price(self, entry_price: float, stop_distance: float, side: TradeSide) -> float:
    ...
    def validate_signal_conditions(self, row: pd.Series) -> Dict[str, Any]:
        ...
        try:
            conditions = {
                ...
                'price_above_ma_short': bool(row.get('close', 0) > row.get('ma_short', 0)),
                'price_below_ma_short': bool(row.get('close', 0) < row.get('ma_short', 0)),
    ...
    def get_signal_strength(self, row: pd.Series, side: TradeSide) -> float:
        ...
        try:
            if side == TradeSide.LONG:
                ...
                price_strength = 1.0 if row.get('close', 0) > row.get('ma_short', 0) else 0.0
                rsi_strength = max(0, (self.config.rsi_ob - row.get('rsi', 50)) / self.config.rsi_ob)
                session_strength = 1.0 if row.get('in_session', True) else 0.0
                vol_strength = 1.0 if row.get('vol_ok', True) else 0.0
                
                # Weighted average
                strength = (trend_strength * 0.3 + price_strength * 0.2 + 
                           rsi_strength * 0.2 + session_strength * 0.15 + vol_strength * 0.15)
            
            else:  # SHORT
                # Short signal strength factors
                trend_strength = 1.0 if row.get('trend_dn', False) else 0.0
                price_strength = 1.0 if row.get('close', 0) < row.get('ma_short', 0) else 0.0
                rsi_strength = max(0, (row.get('rsi', 50) - self.config.rsi_os) / (100 - self.config.rsi_os))
                session_strength = 1.0 if row.get('in_session', True) else 0.0
                vol_strength = 1.0 if row.get('vol_ok', True) else 0.0
    ...
    def get_cooldown_status(self) -> Dict[str, Any]:
        ...
        bars_since_exit = self.current_bar - self.last_exit_bar if self.last_exit_bar != -999 else 0
...
# Export for DI
__all__ = ['SignalDetectionEngine', 'SignalConfig', 'TradeSide', 'create_signal_detection_engine']
