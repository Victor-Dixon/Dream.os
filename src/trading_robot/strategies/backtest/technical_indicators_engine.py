...
class TechnicalIndicatorsEngine:
    ...
    def calculate_custom_indicator(self, df: pd.DataFrame, indicator_name: str, **kwargs) -> pd.DataFrame:
    ...
    def get_indicator_values(self, df: pd.DataFrame, indicator: str, row_idx: int) -> Optional[float]:
    ...
    def get_indicator_summary(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        indicator_columns = ['ma_short', 'ma_long', 'rsi', 'atr', 'trend_up', 'trend_dn', 'vol_ok', 'in_session']
    ...
    def validate_indicators(self, df: pd.DataFrame) -> bool:
        ...
        required_indicators = ['ma_short', 'ma_long', 'rsi', 'atr', 'trend_up', 'trend_dn', 'vol_ok', 'in_session']
        ...
        if 'rsi' in df.columns:
            ...
            if len(rsi_values) > 0:
                if rsi_values.min() < 0 or rsi_values.max() > 100:
                    print(f"Error: RSI values out of range: min={rsi_values.min()}, max={rsi_values.max()}")
                    return False
...
def create_technical_indicators_engine(config: IndicatorConfig) -> TechnicalIndicatorsEngine:
    """Factory function to create technical indicators engine with configuration"""
    return TechnicalIndicatorsEngine(config)


# Export for DI
__all__ = ['TechnicalIndicatorsEngine', 'IndicatorConfig', 'create_technical_indicators_engine']
