...
class TSLAATRBacktestOrchestrator:
    ...
    def __init__(self, config: Optional[BacktestConfig] = None):
        ...
        analytics_config = AnalyticsConfig(initial_capital=self.config.initial_capital)
            ...
            for idx, row in self.data.iterrows():
                ...
                if self.position_engine.position_size != 0:
                    exit_reason = self.signal_engine.check_exit_signals(
                        ...
                        (
                            self.position_engine.current_trade.stop_price
                            if self.position_engine.current_trade
                            else 0
                        ),
                        (
                            self.position_engine.current_trade.target_price
                            if self.position_engine.current_trade
                            else 0
                        ),
                    )
            ...
            results.update(
                {
                    ...
                    "total_return_pct": (
                        (
                            (
                                self.position_engine.current_capital
...
def create_tsla_atr_backtest_orchestrator(
    config: Optional[BacktestConfig] = None,
) -> TSLAATRBacktestOrchestrator:
    """Factory function to create TSLA ATR backtest orchestrator with optional
    configuration."""
