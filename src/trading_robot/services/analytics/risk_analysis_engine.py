...
class RiskAnalysisEngine:
    ...
    def calculate_risk_metrics(self, trades: List[Trade], portfolio_value: float) -> RiskMetrics:
        ...
        try:
            ...
            var_95 = self._calculate_value_at_risk(returns, self.config.var_confidence_level)
            expected_shortfall = self._calculate_expected_shortfall(returns, self.config.var_confidence_level)
    ...
    def _assess_risk_level(self, volatility: float, var_95: float) -> RiskLevel:
        ...
        if var_95 > self.config.critical_var_threshold or volatility > self.config.critical_volatility_threshold:
            return RiskLevel.CRITICAL
        elif var_95 > self.config.high_var_threshold or volatility > self.config.high_volatility_threshold:
            return RiskLevel.HIGH
        elif var_95 > self.config.medium_var_threshold or volatility > self.config.medium_volatility_threshold:
...
def create_risk_analysis_engine(config: Optional[RiskAssessmentConfig] = None) -> RiskAnalysisEngine:
