...
class GitHubBookNavigator(discord.ui.View):
    ...
    def _setup_jump_dropdown(self):
        ...
        if options:
            select = discord.ui.Select(
                placeholder="🔍 Jump to repo...", options=options, custom_id="jump_select", row=1
            )
    ...
    def _create_repo_embed(self) -> discord.Embed:
        ...
        is_goldmine = any(
            keyword in content for keyword in ["GOLDMINE", "JACKPOT", "goldmine", "jackpot"]
        )
        ...
        embed.set_footer(
            text=f"📚 GitHub Book: {analyzed}/75 analyzed ({analyzed/total*100:.1f}%) | Use buttons to navigate"
        )
    ...
    def _add_repo_fields(self, embed: discord.Embed, repo_data: dict[str, Any]):
        ...
        if repo_data.get("integration_hours") and repo_data["integration_hours"] != "N/A":
            embed.add_field(
                name="⏱️ Integration", value=f"{repo_data['integration_hours']} hours", inline=True
            )
        ...
        if repo_data.get("recommendations"):
            embed.add_field(
                name="💭 Recommendation", value=repo_data["recommendations"][:1024], inline=False
            )
    ...
    def _extract_integration_effort(self, content: str) -> str:
        ...
        integration_lines = [
            ...
            and any(word in line for word in ["Integration", "Effort", "ROI"])
        ]
    ...
    def _extract_quality_roi(self, content: str) -> str:
        ...
        quality_lines = [
            line
            for line in lines
            if any(word in line for word in ["Quality", "ROI", "Rating", "⭐"])
        ]
...
class GitHubBookCommands(commands.Cog if DISCORD_AVAILABLE else object):
    ...
    async def show_github_book(self, ctx: commands.Context, repo_num: int | None = None):
        """
    ...
    async def search_repos(self, ctx: commands.Context, *, keyword: str):
        ...
        try:
            ...
            if len(matches) > 10:
                embed.set_footer(
                    text=f"Showing 10 of {len(matches)} results | Refine search for better results"
                )
    ...
    async def filter_by_agent(self, ctx: commands.Context, agent_id: str | None = None):
        """
