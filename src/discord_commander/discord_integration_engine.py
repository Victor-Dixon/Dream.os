...
class DiscordIntegrationEngine:
    ...
    async def initialize_channels(self, guild: discord.Guild) -> bool:
        ...
        try:
            ...
            if str(guild.id) != self.config.guild_id:
                self.logger.warning(
                    f"Guild ID mismatch: expected {self.config.guild_id}, got {guild.id}"
                )
                return False
    ...
    async def send_startup_message(self) -> bool:
        ...
        try:
            ...
            status_channel = discord.utils.get(
                self.guild.channels, name=self.config.status_channel
            )
            if not status_channel:
                self.logger.error(
                    f"Status channel '{self.config.status_channel}' not found"
                )
                return False
            ...
            embed.fields = [
                {"name": "Status", "value": "✅ Operational", "inline": True},
                {
                    "name": "Active Agents",
                    "value": (
                        f"{len(self.swarm_status.active_agents)}/{self.swarm_status.total_agents}"
                    ),
                    "inline": True,
                },
                {
                    "name": "Current Cycle",
                    "value": f"Cycle {self.swarm_status.current_cycle}",
                    "inline": True,
                },
                {
                    "name": "System Health",
                    "value": self.swarm_status.system_health,
                    "inline": True,
                },
                {
                    "name": "Efficiency Rating",
                    "value": f"{self.swarm_status.efficiency_rating}x",
                    "inline": True,
                },
            ]
    ...
    def create_status_embed(self) -> discord.Embed:
        ...
        embed.fields = [
            ...
            },
            {
                ...
                "value": str(len(self.swarm_status.active_missions)),
                "inline": True,
            },
            {
                "name": "Pending Tasks",
                "value": str(len(self.swarm_status.pending_tasks)),
                "inline": True,
            },
        ]
    ...
    def has_permission(self, member: discord.Member, permission: str) -> bool:
        ...
        if permission == "agent":
            agent_roles = [
                self.get_role_by_name(role) for role in self.config.agent_roles
            ]
...
def create_discord_integration_engine(
    config: DiscordConfig, swarm_status: SwarmStatus
