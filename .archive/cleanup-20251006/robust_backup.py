...
def create_robust_backup():
    ...
    print("🔄 Creating robust backup of web infrastructure...")
    ...
    if web_orig > 0 and web_backup_count == 0:
        print("❌ Web backup completely failed")
        integrity_check = False
    elif web_orig > 0 and abs(web_orig - web_backup_count) > 5:  # Allow small differences
        print("⚠️  Web backup significant file count difference")
        integrity_check = False
    elif web_orig > 0:
        print("✅ Web backup integrity verified")

    if infra_orig > 0 and infra_backup_count == 0:
        print("❌ Infrastructure backup completely failed")
        integrity_check = False
    elif infra_orig > 0 and abs(infra_orig - infra_backup_count) > 5:
        print("⚠️  Infrastructure backup significant file count difference")
        integrity_check = False
    elif infra_orig > 0:
        print("✅ Infrastructure backup integrity verified")

    if integrity_check and (web_backup_count > 0 or infra_backup_count > 0):
        print("🎉 CYCLE 1 - ROBUST BACKUP COMPLETE: Enterprise-grade backup created!")
        ...
        print("❌ Backup integrity issues detected - please verify manually")
        if error_log:
            print("Error details:")
            for error in error_log[:3]:  # Show first 3 errors
                print(
                    f'  - {error.get("source", "Unknown")}: {error.get("error", "Unknown error")[:100]}...'
                )
        return False
