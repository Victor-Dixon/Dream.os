...
- ✅ **tools/**: 53 files, ~200+ tools, WELL-ORGANIZED
...
### **DECISION 1: Single Source of Truth - tools/**

**Mandate:** `tools/` is the OFFICIAL toolbelt system going forward
...
**Action:**
- ✅ Keep `tools/toolbelt_core.py` as primary
- ❌ Deprecate `tools/toolbelt.py` (legacy)
- ❌ Deprecate `tools/agent_toolbelt.py` (redundant)
- ✅ Migrate all orphans to `tools/` categories
...
**Implementation:**
1. Create adapters in existing tools categories
...
- Duplicate functionality (already covered by tools)
- Obsolete tools (no longer used)
- One-off scripts (specific to old tasks)

**Integration Process:**
1. For each tool, create adapter in appropriate tools category
2. Register in tool_registry.py
3. Add to toolbelt_core.py search index
4. Test via `python -m tools.toolbelt <tool_name>`
...
- tools/toolbelt_core.py (modern, 300+ lines)

**Architectural Decision:**

**PRIMARY TOOLBELT:** `tools/toolbelt_core.py`
...
print("⚠️ DEPRECATED: Use 'python -m tools.toolbelt' instead")
print("⚠️ This file will be removed in future version")
# Then delegate to tools
from tools.toolbelt_core import main
main()
```

**Phase 2 (Next cycle):**
- Update all references in docs to use tools
- Update all scripts to import from tools
- Create migration guide

**Phase 3 (Future):**
- Remove tools/toolbelt.py completely
- Archive tools/agent_toolbelt.py
- Single entry point: `python -m tools.toolbelt`

**SINGLE COMMAND INTERFACE:**
```bash
# Primary interface
python -m tools.toolbelt <tool_name> [args]

# Search tools
python -m tools.toolbelt --search <query>

# List all tools
python -m tools.toolbelt --list

# Show help
python -m tools.toolbelt --help <tool_name>
```
...
2. Migrate 15+ captain tools to tools categories
...
2. Update all documentation to reference tools
...
import warnings
warnings.warn(
    "⚠️ DEPRECATED: Use 'python -m tools.toolbelt <tool>' instead. "
...
3. Create tools adapters if missing

4. Test functionality preserved

**Task 2: Begin Captain Tools Migration (1-2 hours)**
1. Create `tools/categories/captain_coordination_tools.py`
