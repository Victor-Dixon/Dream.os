...
TOOL_REGISTRY: dict[str, tuple[str, str]] = {
    ...
    "infra.extract_planner": (
        "tools_v2.categories.infrastructure_tools",
    ...
    "advisor.recommend": (
        "tools_v2.categories.intelligent_mission_advisor_adapter",
        "MissionAdvisorTool",
    ),
    "advisor.validate": (
        "tools_v2.categories.intelligent_mission_advisor_adapter",
    ...
    "integration.find-ssot-violations": (
        "tools_v2.categories.integration_tools",
    ...
    "coord.request-review": (
        "tools_v2.categories.coordination_tools",
...
class ToolRegistry:
    ...
    def resolve(self, tool_name: str) -> type[IToolAdapter]:
        ...
        if tool_name not in TOOL_REGISTRY:
            ...
            raise ToolNotFoundError(
                f"Tool '{tool_name}' not found. Available: {available}", tool_name=tool_name
            )
        ...
        try:
            ...
            raise ToolNotFoundError(
                f"Failed to import tool '{tool_name}': {e}", tool_name=tool_name
            )
    ...
    def export_lock(self, output_path: str = "tools_v2/tool_registry.lock.json") -> None:
        ...
        """
        lock_data = {"version": "2.0.0", "tools": TOOL_REGISTRY, "count": len(TOOL_REGISTRY)}
