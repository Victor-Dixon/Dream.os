...
def generate_readiness_report(repos: List[str]) -> str:
    ...
    for repo in repos:
        ...
        status = "✅ READY" if all_steps_complete else "⏳ NOT READY"
        all_ready = all_ready and all_steps_complete
        
        report_lines.extend([
            f"\n📦 {repo}",
            f"Status: {status}",
            f"  Step 3 (Integration Planning): {'✅' if readiness['integration_planning'] else '❌'}",
            f"  Step 5 (Duplicate Resolution): {'✅' if readiness['duplicate_resolution'] else '❌'}",
            f"  Step 6 (Venv Cleanup): {'✅' if readiness['venv_cleanup'] else '❌'}",
            f"  Step 7 (Integration Review): {'✅' if readiness['integration_review'] else '❌'}",
        ])
    
    report_lines.extend([
        "",
        "=" * 60,
        f"Overall Status: {'✅ ALL READY' if all_ready else '⏳ SOME NOT READY'}",
        "=" * 60,
    ])
