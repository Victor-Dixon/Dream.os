...
⚠️  DEPRECATED: This tool has been replaced by the validation-audit MCP server
   Use the 'validation-audit' MCP server for all validation and audit operations instead.
...
class ClosureValidator:
    ...
    def validate(self, closure_file_path: Optional[Path] = None) -> Tuple[bool, List[str], List[str]]:
        """
        Validate closure format.
        
        Args:
            closure_file_path: Optional path to closure file (for audit evidence check)
        ...
        # Check audit evidence (if file path provided)
        if closure_file_path:
            audit_error = self._check_audit_evidence(closure_file_path)
            if audit_error:
                self.violations.append(f'Audit evidence check failed: {audit_error}')
    ...
    def _check_audit_evidence(self, closure_file_path: Path) -> Optional[str]:
        """Check for working tree audit evidence file.
        
        Returns error message if audit evidence is missing or invalid, None if OK.
        """
        # Find latest audit evidence file
        reports_dir = closure_file_path.parent.parent / "reports"
        if not reports_dir.exists():
            return "Reports directory not found - audit evidence missing"
        
        audit_files = sorted(glob.glob(str(reports_dir / "working_tree_audit_*.json")), reverse=True)
        if not audit_files:
            return "No working tree audit evidence found - run working_tree_audit.py first"
        
        latest_audit = Path(audit_files[0])
        
        try:
            with open(latest_audit, 'r', encoding='utf-8') as f:
                audit_data = json.load(f)
        except Exception as e:
            return f"Failed to read audit evidence: {e}"
        
        # Check timestamp freshness (within 1 hour of closure)
        audit_ts_str = audit_data.get("ts", "")
        if not audit_ts_str:
            return "Audit evidence missing timestamp"
        
        try:
            # Parse timestamp format: YYYYMMDD_HHMMSS
            audit_time = datetime.strptime(audit_ts_str, "%Y%m%d_%H%M%S")
            closure_time = datetime.fromtimestamp(closure_file_path.stat().st_mtime)
            
            # Check if audit is within 1 hour of closure
            time_diff = abs((closure_time - audit_time).total_seconds())
            if time_diff > 3600:
                return f"Audit evidence stale ({(time_diff/3600):.1f} hours old)"
        except Exception as e:
            return f"Failed to parse audit timestamp: {e}"
        
        # Check foreign paths handling
        foreign_paths = audit_data.get("foreign_paths", {})
        task_ids = audit_data.get("task_ids", {})
        broadcast_sent = audit_data.get("broadcast_sent", False)
        
        if foreign_paths:
            # All foreign path buckets must have task IDs
            missing_tasks = [bucket for bucket in foreign_paths.keys() if bucket not in task_ids]
            if missing_tasks:
                return f"Foreign paths detected but missing task IDs for: {', '.join(missing_tasks)}"
            
            # Broadcast must be sent if foreign paths exist
            if not broadcast_sent:
                return "Foreign paths detected but broadcast not sent"
...
def validate_file(file_path: Path) -> Tuple[bool, List[str], List[str]]:
    ...
    return validator.validate(closure_file_path=file_path)
