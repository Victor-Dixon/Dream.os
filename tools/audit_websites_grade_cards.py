...
def audit_website(website: str, sites_dir: Path) -> Dict:
    ...
    if not website_dir.exists():
        issues["technical"].append(f"Website directory not found: {website_dir}")
        technical_score = 0
    else:
        ...
        if tasks_active.exists():
            with open(tasks_active, 'r', encoding='utf-8') as f:
                ...
                if "None" not in content or len(content) > 50:
                    ...
                    tasks["high"].append(f"Add active content/tasks for {website}")
        ...
        if tasks_backlog.exists():
            with open(tasks_backlog, 'r', encoding='utf-8') as f:
                ...
                if "lead" in content or "capture" in content or "booking" in content or "payment" in content:
                    ...
                    issues["business"].append("No business readiness tasks identified")
                    tasks["high"].append(f"Add business readiness tasks for {website}")
...
"""
    ...
    for result in audit_results:
        grade_counts[result["overall_grade"]] = grade_counts.get(result["overall_grade"], 0) + 1
