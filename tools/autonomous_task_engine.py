    ...
    def generate_autonomous_report(self, agent_id: str) -> str:
        ...
        for i, rec in enumerate(recommendations, 1):
            ...
            if task.current_lines and task.target_lines:
                report.append(f"Lines: {task.current_lines}→{task.target_lines} ({task.reduction_percent:.0f}% reduction)")
            ...
            report.append("")
            report.append(f"TO CLAIM: python tools/autonomous_task_engine.py --claim {task.task_id} --agent {agent_id}")
            report.append("-" * 80)
            report.append("")
