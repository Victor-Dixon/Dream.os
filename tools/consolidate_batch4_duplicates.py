...
def main():
    ...
    for i, group in enumerate(batch4_groups, 1):
        ...
        if not ssot_result["valid"]:
            print(f"   ⚠️  SSOT verification failed: {ssot_result.get('error', 'Unknown error')}")
            ssot_failed += 1
            continue
        ...
        if duplicates_result["missing"]:
            print(f"   ⚠️  {len(duplicates_result['missing'])} duplicate(s) already missing")
        
        if duplicates_result["different"]:
            print(f"   ⚠️  {len(duplicates_result['different'])} duplicate(s) have different content (skipping)")
