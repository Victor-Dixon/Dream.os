...
import argparse, os, re, sys, json, hashlib, time, subprocess, fnmatch
...
def main():
    ...
    # duplicate detection (cap if needed)
    ...
    # Risk guard – if git available, estimate previous .py count via history? (best effort = current only)
    py_count = len(py_files)
    # Soft heuristic: if we see suspiciously low py_count, warn/fail
    risk_flag = False
    risk_msgs: List[str] = []
    if py_count < RISK_MIN_PY:
        risk_flag = True
        risk_msgs.append(f".py count dangerously low: {py_count} < {RISK_MIN_PY}")
    # If git present, try rough drop estimate using HEAD tree (fast ls-tree)
    py_head = None
    if git_available():
        try:
    ...
    if risk_flag and not args.force:
        print(f"❌ Loss guard triggered. See {md_path}")
        return 1
    if risk_flag and args.force:
        print(f"⚠️ Proceeding under --force. See {md_path}")
        print(json_path)
        return 2
