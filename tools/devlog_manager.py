...
Implementation: shell out to canonical scripts to avoid import collisions with
`src/tools` (package) vs repo-root `tools/`.
...
def main() -> int:
    parser = argparse.ArgumentParser(prog="devlog_manager", description="Manage devlogs (post/validate).")
    ...
    if args.command == "validate":
        cmd = [
            sys.executable,
            "tools/devlog_compliance_validator.py",
            "--agent",
            args.agent,
            "--file",
            args.file,
