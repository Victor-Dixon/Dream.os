...
def generate_health_report(
    ...
    report += "## 📚 **Documentation Availability**\n\n"
    ...
    if repo_state:
        ...
        if repo_state.get("exists"):
            report += f"- **Exists**: ✅\n"
            report += f"- **Git Repo**: {'✅' if repo_state.get('is_git_repo') else '❌'}\n"
            report += f"- **Python Files**: {'✅' if repo_state.get('has_python_files') else '❌'} ({repo_state.get('python_file_count', 0)} files)\n"
            report += f"- **Test Files**: {'✅' if repo_state.get('has_tests') else '❌'} ({repo_state.get('test_file_count', 0)} files)\n"
            report += f"- **Venv Files**: {'⚠️' if repo_state.get('has_venv_files') else '✅'}\n"
        else:
            report += "- **Exists**: ❌\n"
    ...
    if overall_health >= 90:
        report += "**Status**: 🟢 **Excellent** - Ready for integration work\n"
    elif overall_health >= 70:
        report += "**Status**: 🟡 **Good** - Minor gaps, ready for integration\n"
    elif overall_health >= 50:
        report += "**Status**: 🟠 **Fair** - Some gaps, review needed\n"
    else:
        report += "**Status**: 🔴 **Poor** - Significant gaps, setup needed\n"
...
def main():
    """Main execution."""
    print("=" * 60)
    print("🔍 INTEGRATION HEALTH CHECKER")
    print("=" * 60)
    print()
    ...
    tool_count = sum(1 for v in tools.values() if v)
    print(f"   ✅ {tool_count}/{len(tools)} tools available")
    print()
    ...
    doc_count = sum(1 for v in docs.values() if v)
    print(f"   ✅ {doc_count}/{len(docs)} key documents available")
    print()
    ...
    if len(sys.argv) > 1:
        ...
        print(f"   ✅ Repository check complete")
        print()
