...
def analyze_init_files(root_dir: str = "src") -> dict:
    ...
    simple_imports = [
        f for f in file_data if f["has_imports"] and not f["has_all"] and f["lines"] < 10
    ]
...
def print_analysis(analysis: dict):
    ...
    total_removable = (
        ...
        + int(
            analysis["categories"]["simple_imports"] * 0.7
        )  # Estimate 70% of simple imports can be consolidated
    )
...
def main():
    ...
    print("Phase 1: Remove Empty Files")
    ...
    print("Phase 2: Remove Duplicate Files")
    ...
    print("Phase 3: Consolidate Simple Imports")
    print(
        f"  • Count: ~{int(len(plan['phase_3_consolidate_simple']) * 0.7)} files (70% of {len(plan['phase_3_consolidate_simple'])})"
    )
    print()
    ...
    print(
        f"  • Reduction: ~{plan['estimated_reduction']} files ({plan['reduction_percentage']:.1f}%)"
    )
    print()
    ...
    with open(output_file, "w") as f:
        json.dump({"analysis": analysis, "plan": plan}, f, indent=2)
