        ...
        for pattern in config_patterns:
            for config_file in repo_path.rglob(pattern):
                if "__pycache__" not in str(config_file):
                    self.config_files[repo_name].append(str(config_file.relative_to(repo_path)))
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        default=["agentproject", "Auto_Blogger", "crosbyultimateevents.com", "contract-leads", "Thea"],
    ...
    if args.check_isolation:
        isolated_count = sum(1 for r in results["dependency_isolation"] if r["isolated"])
        print(f"Dependency Isolation: {isolated_count}/{len(results['dependency_isolation'])} repos isolated")
    
    if args.check_direction:
        has_cycles = results["dependency_direction"].get("has_circular_dependencies", False)
        print(f"Dependency Direction: {'❌ Has circular dependencies' if has_cycles else '✅ No circular dependencies'}")
        if has_cycles:
            for cycle in results["dependency_direction"].get("circular_dependencies", [])[:5]:
                print(f"  - {cycle}")
    
    if args.check_config:
        valid_count = sum(1 for r in results["configuration_management"] if r["valid"])
        print(f"Configuration Management: {valid_count}/{len(results['configuration_management'])} repos valid")
